...
export class ProfileComponent implements OnInit {
  ...
  constructor(private dataService: EmployeeServiceService,public dialog: MatDialog) {
    ...
    this.fullName = this.dataService.employeeRecord[key].firstName.concat(' ',this.dataService.employeeRecord[key].lastName);
    this.jobTitle = this.dataService.employeeRecord[key].jobTitle.split('_').join(' ');
    this.department = this.dataService.employeeRecord[key].department.split('_').join(' ');
    this.email = this.dataService.employeeRecord[key].email;
    this.location = this.dataService.employeeRecord[key].location.split('_').join(' ');
